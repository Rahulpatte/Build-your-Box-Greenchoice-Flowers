...
function Bundle() {
  ...
  const resourcePicker = async (singleSelection = false) => {
    try {
      // Open resource picker first without triggering the loader
      ...
      // When the selection is made (e.g., after the "Add" button is clicked), show the loader
      ...
      if (collectionId !== 0) {
        // Edit mode: merge new product IDs with existing `editedproductIds`
        ...
        // Update product IDs, titles, and images
        setProductIds(mergedProductIds);

        // Update UI with titles and images after fetching all product data
      ...
      // Turn off loader in case of error
      setCreateBundleLoader(false);
  ...
  const handleSubmit = async () => {
    ...
    const productsToUpdate =
      productIds.length > 0 ? productIds : editedproductIds;
  ...
  // const[handleEditproducts,setHandleEditproducts]=useState(false)
  ...
  //   const newVariantIds = selectedProduct[0].variants.map((el) => el.id);
  //   console.log("newVarientIds",newVariantIds);

  //   const updatedProductIds = [...editedproductIds];
  //   console.log("updatedProductIds",updatedProductIds);

  //   updatedProductIds[index] = newVariantIds.length > 1 ? newVariantIds : newVariantIds[0];

  //   console.log("updatedProductIds",updatedProductIds);

  //   setHandleEditids(updatedProductIds)

  //   // fetchGraphQLData(updatedProductIds)

  //   // console.log(fetchGraphQLData(updatedProductIds))
  ...
  const rows = paginatedTitles.map((title, index) => (
    ...
    >
      <IndexTable.Cell>{actualIndex(index) + 1}</IndexTable.Cell>{" "}
      ...
      <IndexTable.Cell>
        <InlineStack gap="500">
          ...
          {/* <EditIcon/> */}
          <Button onClick={() => handleDelete(actualIndex(index))}>
            Delete
          </Button>
        </InlineStack>
      </IndexTable.Cell>
